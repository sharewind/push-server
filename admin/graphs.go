...
func NewGraphOptions(rw http.ResponseWriter, req *http.Request,
	...
	base := context.admin.options.GraphiteURL
	if context.admin.options.ProxyGraphite {
		base = ""
	}
	o := &GraphOptions{
		context:           context,
		Configured:        context.admin.options.GraphiteURL != "",
		Enabled:           g.Timeframe != "off" && context.admin.options.GraphiteURL != "",
		GraphiteUrl:       base,
...
func (g *GraphOptions) Prefix(host string, metricType string) string {
	...
	prefixWithHost := strings.Replace(g.context.admin.options.StatsdPrefix, "%s", statsdHostKey, -1)
	if prefixWithHost[len(prefixWithHost)-1] != '.' {
		prefixWithHost += "."
	}
	if g.context.admin.options.UseStatsdPrefixes && metricType == "counter" {
		prefix += "stats_counts."
	} else if g.context.admin.options.UseStatsdPrefixes && metricType == "gauge" {
		prefix += "stats.gauges."
	}
...
func parseRateResponse(body []byte) ([]byte, error) {
	...
	if err != nil {
		log.Debug("ERROR: failed to parse metadata - %s", err.Error())
		return nil, err
	}
